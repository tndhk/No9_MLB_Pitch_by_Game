...
class BaseballSavantClient:
    ...
    def search_pitcher(self, name: str) -> List[Pitcher]:
        ...
        # 実際には別のAPIリクエストでIDを検索する必要がある
        # 現在は主要な投手の一部をダミーデータとして返す
        pitcher_dict = {
            ...
            'Yu Darvish': '506433',
            'Kodai Senga': '669011',
            'Yoshinobu Yamamoto': '993772'
        }
        
        # 部分一致で検索
        matched_pitchers = []
        for pitcher_name, pid in pitcher_dict.items():
            if name.lower() in pitcher_name.lower():
                matched_pitchers.append(
                    Pitcher(
                        id=pid,
                        name=pitcher_name,
                        team=None,  # チーム情報の取得にはさらなるAPIリクエストが必要
                        throws=None  # 投球腕情報
                    )
                )
        
        if not matched_pitchers:
            self.logger.warning(f"警告: '{name}'に一致する投手が見つかりませんでした")
        else:
            self.logger.info(f"{len(matched_pitchers)}人の投手が見つかりました")
